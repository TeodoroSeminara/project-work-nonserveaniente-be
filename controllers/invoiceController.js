...
function storeInvoice(req, res) {
    ...
    if (items.some(it => !it.slug || !it.quantity)) {
        return res.status(400).json({ error: "Ogni elemento di items deve avere slug e quantity" });
    }
    ...
    connection.beginTransaction(err => {
        ...
        const productsSql = `
      SELECT id, name, price, slug, quantity
      FROM products
      WHERE slug IN (?)
    `;

        connection.query(productsSql, [productSlugs], (errProd, prodRows) => {
            if (errProd) {
            ...
            if (!prodRows || prodRows.length !== productSlugs.length) {
                return connection.rollback(() => {
                    res.status(400).json({ error: "Uno o più prodotti negli items non esistono" });
                });
            }
            ...
            for (const it of items) {
                ...
                if (!p) {
                    return connection.rollback(() => {
                        res.status(400).json({ error: `Prodotto con slug '${it.slug}' non trovato` });
                    });
                }
                if (isNaN(qty) || qty <= 0) {
                    return connection.rollback(() => {
                ...
                if (qty > p.quantity) {
                    return connection.rollback(() => {
                        res.status(400).json({
                            error: `La quantità richiesta per '${p.name}' (${qty}) supera la disponibilità a magazzino (${p.quantity}).`,
                        });
                    });
                }
          ...
          `;
                    ...
                    lines.forEach((l) => {
                        connection.query(
                            ...
                            (errQty, resultQty) => {
                                if (hasError) return; // errore già gestito

                                if (errQty) {
                                    hasError = true;
                                    return connection.rollback(() => {
                                        res.status(500).json({ error: "Errore nell'aggiornamento della quantità prodotti" });
                                    });
                                }

                                if (resultQty.affectedRows === 0) {
                                    hasError = true;
                                    return connection.rollback(() => {
                                        res.status(400).json({
                                            error: `Stock insufficiente per il prodotto '${l.name_product}'`,
                                        });
                                    });
                                };
                            });
...
// DELETE – elimina fattura + righe product_invoice
